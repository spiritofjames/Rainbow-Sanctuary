...
async function readArticles(sourceDir) {
  const directory = path.join(sourceDir, 'articles');
  const files = (await readdir(directory)).filter((name) => name.endsWith('.md')).sort();
  return Promise.all(files.map(async (name) => {
    const parsed = matter(await readFile(path.join(directory, name), 'utf8'));
...
function renderArticleCard(article, topic) {
  return `<article class="rs-knowledge-article-card"><p class="rs-knowledge-kicker">${escapeHtml(topic.name)} · ${escapeHtml(article.content_type.replaceAll('-', ' '))}</p><h2><a href="${article.canonical_path}">${escapeHtml(article.title)}</a></h2><p>${escapeHtml(article.summary)}</p><small>Updated ${escapeHtml(article.updated_at)}</small></article>`;
}
